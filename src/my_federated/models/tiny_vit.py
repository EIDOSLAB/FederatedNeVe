...
def get_tiny_vit_model(num_classes: int = 10, model_name: str = "tiny_vit_5m_224", use_pretrain: bool = False):
    if model_name == "tiny_vit_5m_224":
        return _tiny_vit_5m_224(num_classes=num_classes, pretrained=use_pretrain)
    if model_name == "tiny_vit_11m_224":
        return _tiny_vit_11m_224(num_classes=num_classes, pretrained=use_pretrain)
    if model_name == "tiny_vit_21m_224":
        return _tiny_vit_21m_224(num_classes=num_classes, pretrained=use_pretrain)
    assert False, f"TinyVit model name: '{model_name}' not managed!"


@register_model
def _tiny_vit_5m_224(num_classes: int, pretrained=False, **kwargs):
    model_kwargs = dict(
    ...
    model = _create_tiny_vit('tiny_vit_5m_224', pretrained, **model_kwargs)
    model.head = nn.Linear(len(model.norm_head.bias), num_classes)
    return model


@register_model
def _tiny_vit_11m_224(pretrained=False, **kwargs):
    model_kwargs = dict(
...
@register_model
def _tiny_vit_21m_224(pretrained=False, **kwargs):
    model_kwargs = dict(
...
@register_model
def _tiny_vit_21m_384(pretrained=False, **kwargs):
    model_kwargs = dict(
        img_size=384,
        embed_dims=[96, 192, 384, 576],
        depths=[2, 2, 6, 2],
...
@register_model
def _tiny_vit_21m_512(pretrained=False, **kwargs):
    model_kwargs = dict(
        img_size=512,
        embed_dims=[96, 192, 384, 576],
        depths=[2, 2, 6, 2],
